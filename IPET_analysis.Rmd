...
bar_plot(means_trust, palette[4:6]) + 
  scale_x_discrete(labels=c("Identify Media Source", "Estimate Helping", "Imagine Helping")) +
  xlab ("Condition") +
  ylab ("Trust")
#assess random effect relative to the means random effect
...
# Scene 
plot.df_scene<-cbind.data.frame("Vividness"=c("Imagine Helping", "Estimate Helping"), 
                           "IE"= c(0.36,0.05),
                           "l_CI"=c(0.24, -0.02), 
                           "u_CI"=c(0.50, 0.14))
...
# Scene 
plot.df_scene2<-cbind.data.frame("Vividness"=c("Imagine helping", "Estimate helping"), 
                           "IE"= c(0.15,0.08),
                           "l_CI"=c(0.08, -0.03), 
                           "u_CI"=c(0.23, 0.20))
ggplot(plot.df_scene2, aes(x=IE, y=Vividness)) +
  ...
  ylab ("Conditions")
```
#MLM factor analysis:emotional reaction using combined dataset
...
```{r}
# Detail -- Model:
Bayes_det_help<-brm(data= interaction.df, family=cumulative("logit"), 
...
#------------------------------------------------------------------------------------------------------------------------
# Coherence -- Model: 
Bayes_coh_help<-brm(data= interaction.df, family=cumulative("logit"), 
...
int_posterior_help <-plot_int_help
...
# pd 
p_direction(int_posterior_help)
# OR
posterior_summary(exp(int_posterior_help), probs=c(0.055, 0.945))
...
```{r}
# Detail -- Model:
Bayes_det_trust<-brm(data= interaction.df, family=cumulative("logit"), 
      bf(trust.response~Condition*Detail_C+(Condition*Detail_C|participant)+(Condition*Detail_C|Story_ID)) +
         lf(disc ~ 0 + Condition, cmc=F), 
      prior= c(prior(normal (0,1), class=b), 
               prior(normal(0,1.2), class=Intercept),
               prior(exponential(1), class= sd),
...
#------------------------------------------------------------------------------------------------------------------------
# Coherence -- Model: 
Bayes_coh_trust<-brm(data= interaction.df, family=cumulative("logit"), 
      bf(trust.response~Condition*Coherence_C+(Condition*Coherence_C|participant)+(Condition*Coherence_C|Story_ID)) +
         lf(disc ~ 0 + Condition, cmc=F), 
      prior= c(prior(normal (0,1), class=b), 
               prior(normal(0,1.2), class=Intercept),
               prior(exponential(1), class= sd),
...
int_posterior_trust <- plot_int_trust
...
f6<-violin_int_trust +
  labs(x="Vivindess by Condition Interactions", y =  "Trust (logit)") +
  theme(legend.position = "none") +
  ylim(-0.7, 1.5) +  
  scale_fill_manual(values=c(palette[6], palette[7])) +
  scale_color_manual(values = c(palette[6], palette[7])) +
...
# pd 
p_direction(int_posterior_trust)
# OR
posterior_summary(exp(int_posterior_trust), probs=c(0.055, 0.945))

grid.arrange(f4,f5, f6, ncol=3)
```
