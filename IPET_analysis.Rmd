...
library (ggplot2)
plot_H<- ggplot(IPET.df, aes(help.response))+
              geom_histogram()
plot_H
plot_T<- ggplot(IPET.df, aes(trust.response))+
              geom_histogram()
plot_T
...
##Repliction analysis: Willingness to help
###Determine if clustering exist
```{r}
#null model from above a compared with null model with an additiion of random effect of story
null_help<- lmer(help.response~(1|participant)+(1|Story_ID), data=IPET.df)
ranova(null_help)#significant so should include story as a random effect
```

###lmer models for main effect
```{r}
...
zcp_model<- lmer(help.response~Condition+(dummy_est+dummy_img||participant)+(dummy_est+dummy_img||Story_ID), data= IPET.df)
...
final_model_help<- lmer(help.response~Condition+(dummy_est+dummy_img|participant)+(1|Story_ID), data= IPET.df)
anova(rm_model,final_model_help, refit=F)
anova(final_model_help, test=' kenward-roger')
```

####Post hoc comparison
```{r}
emmeans(final_model_help, pairwise~Condition,adjust='bonferroni')
confint(emmeans(final_model_help, pairwise~Condition, adjust='bonferroni'))
```
####Plot
```{r}
#Condition difference in willingness to help
ggpredict(final_model_help, c('Condition'))%>% plot()
#assess random effect relative to the means random effect
plot_model(final_model_help, type= 're')

#Identify as reference group
final_model_help2<- lmer(help.response~dummy_est+dummy_img+(dummy_img+dummy_est|participant)+(1|Story_ID), data= IPET.df)
Plot_LMM(final_model_help2, 'Identify', 'Estimate', 'Imagine',1)
Plot_LMM(final_model_help2, 'Identify', 'Estimate', 'Imagine',2)
Plot_LMM(final_model_help2, 'Identify', 'Estimate', 'Imagine',3)
Plot_LMM(final_model_help2, 'Identify', 'Estimate', 'Imagine',4)
#Imagine as reference group
final_model_help3<- lmer(help.response~dummy_ident+dummy_img+(dummy_img+dummy_ident|participant)+(1|Story_ID), data= IPET.df)
Plot_LMM(final_model_help3, 'Estimate', 'Identify', 'Imagine',1)
Plot_LMM(final_model_help3, 'Estimate', 'Identify', 'Imagine',2)
Plot_LMM(final_model_help3, 'Estimate', 'Identify', 'Imagine',3)
Plot_LMM(final_model_help3, 'Estimate', 'Identify', 'Imagine',4)
```
####Effect size
```{r}
r.squaredGLMM(final_model_help)
```

####Assumption testing
```{r}
plot_model (final_model_help, type ='diag')
```
##Trust
###Determine if clustering exist
```{r} 
#null model from above a compared with null model with an additiion of random effect of story
null_trust<- lmer(trust.response~(1|participant)+(1|Story_ID), data=IPET.df)
ranova(null_trust)#significant so should include story as a random effect
```

###lmer models for main effect
```{r}
zcp_trust<- lmer(trust.response~Condition+(dummy_est+dummy_img||participant)+(dummy_est+dummy_img||Story_ID), data= IPET.df)
step(zcp_trust)#suggest that dummy_est in story_ID and participantshould be deleted. 
rm_trust<- lmer(trust.response~Condition+(dummy_img||participant)+(dummy_img||Story_ID), data= IPET.df)
rm_trust<- update(rm_trust, .~., start= getME(rm_trust, 'theta')) #refit twice
#add in correlation parameter
...
anova(fm_trust, test='kenward-roger')
```
####Post hoc comparison
```{r}
emmeans(fm_trust, pairwise~Condition, adjust='bonferroni')
confint (emmeans(fm_trust, pairwise~Condition, adjust='bonferroni'))
```

####Plot
```{r}
#Condition difference in willingness to help
ggpredict(fm_trust, c('Condition'))%>% plot()
...
Plot_LMM(fm_trust2, 'Identify', 'Estimate', 'Imagine',3)
...
Plot_LMM(fm_trust3, 'Estimate', 'Identify', 'Imagine',3)
...
```{r}
#Help
pcurve_help <- powerCurve(final_model_help, test=fcompare(help.response~1), along='participant')
plot(pcurve_help)
#Trust
pcurve_trust<-powerCurve(fm_trust, test=fcompare(trust.response~1), along='participant')
plot(pcurve_trust)
```
...
```{r}
#Detail 
help_zcp_det<- lmer (help.response~Condition*Detail_C+(dummy_img*Detail_C||participant)+(dummy_img*Detail_C||Story_ID),data= interaction.df)
step(help_zcp_det)#everything from Story ID and  Detail_C in Participant removed
...
help_fm_det<- update(help_fm_det, .~., start= getME(help_fm_det, 'theta')) #refit once
anova(help_rm_det,help_fm_det,refit=F)
anova(help_fm_det, test='kenward-roger');summary(help_fm_det)

#Coherence
help_zcp_coh<- lmer (help.response~Condition*Coherence_C+(dummy_img*Coherence_C||participant)+(dummy_img*Coherence_C||Story_ID),
                    data= interaction.df)
step(help_zcp_coh) #remove all from story ID apart from Coherence_C, remove all from Participant apart from dummy_img
...
anova(help_fm_coh, test='kenward-roger');summary(help_fm_coh)

#Scene
help_zcp_scene<- lmer (help.response~Condition*Scene_C+(dummy_img*Scene_C||participant)+(dummy_img*Scene_C||Story_ID),
                    data= interaction.df)
step(help_zcp_scene)
...
anova(help_fm_scene, test='kenward-roger');summary(help_fm_scene)
...
test<- lmer(Detail.response~Condition+(dummy_img|participant)+(0+dummy_img|Story_ID), data= interaction.df)
summary(test)
test<- update(test, .~., start= getME(test, 'theta')) #refit twice
emmeans(test, pairwise~Condition, adjust= 'bonferroni')
test2<- lmer(Coherence.response~Condition+(dummy_img|participant)+(0+dummy_img|Story_ID), data= interaction.df)
summary(test2)
emmeans(test2, pairwise~Condition, adjust= 'bonferroni')
```
...
```{r}
#Detail
trust_det_zcp<- lmer (trust.response~Condition*Detail_C+(dummy_img*Detail_C||participant)+(dummy_img*Detail_C||Story_ID), data=interaction.df)
trust_det_step<- step(trust_det_zcp)#remove dummy_img:detail_C from participant and Detail_C from story ID
...
trust_rm_det<- update(trust_rm_det, .~., start= getME(trust_rm_det, 'theta')) #refit twice
#Add correlation
trust_fm_det<- lmer (trust.response~Condition*Detail_C+(dummy_img+Detail_C|participant)+(dummy_img+dummy_img:Detail_C|Story_ID), data=interaction.df)
summary(rePCA(trust_fm_det)); summary(trust_fm_det) #remove interaction and dummy_img from story_ID
#Final model
...
trust_fm_det<- update(trust_fm_det, .~., start= getME(trust_fm_det, 'theta')) #refit twice
anova(trust_rm_det,trust_fm_det, refit=F)
summary(trust_fm_det);anova(trust_fm_det, test='kenward-roger')

#Coherence
trust_zcp_coh<- lmer (trust.response~Condition*Coherence_C+(dummy_img*Coherence_C||participant)+(dummy_img*Coherence_C||Story_ID), data=interaction.df)
trust_step_coh<- step(trust_zcp_coh) #remove both interactions in participants and story ID as well as coherence_C from story ID
...
trust_coh_rm<- update(trust_coh_rm, .~., start= getME(trust_coh_rm, 'theta')) #one refit
#add correlation 
trust_fm_coh<- lmer (trust.response~Condition*Coherence_C+(dummy_img+Coherence_C|participant)+(dummy_img|Story_ID), data=interaction.df)
summary(rePCA(trust_fm_coh)); summary(trust_fm_coh) #remove dummy_img from story_ID
#final model
trust_fm_coh<- lmer (trust.response~Condition*Coherence_C+(dummy_img+Coherence_C|participant)+(0+dummy_img|Story_ID), data=interaction.df)
trust_fm_coh<- update(trust_fm_coh, .~., start= getME(trust_fm_coh, 'theta'))#one refit
anova(trust_coh_rm,trust_fm_coh, refit=F)
summary(trust_fm_coh);anova(trust_fm_coh, test='kenward-roger')


#Scene
trust_fm_scene<- lmer (trust.response~Condition*Scene_C+(dummy_img+Scene_C|participant)+(0+dummy_img|Story_ID), data=interaction.df)
summary(trust_fm_scene)
```
...
m<-mean(interaction.df$Detail_C)
m-sd;m;m+sd;
ggpredict(trust_fm_det, c("Condition","Detail_C"))%>% plot(rawdata=T)
...
med.fit_0v2_zcpHP<- lmer(Perspective.response~dummy_est +dummy_img+(dummy_est +dummy_img||participant), data= IPET.df)
step(med.fit_0v2_zcpHP) #remove dummy_est 
#Final model 
med.fit_0v2_finalHP<- lmer(Perspective.response~dummy_est +dummy_img+(dummy_img|participant), data= IPET.df)
summary(med.fit_0v2_finalHP)

#Estimate vs Imagine
med.fit_1v2_zcpHP<- lmer(Perspective.response~dummy_ident +dummy_img+(dummy_ident +dummy_img||participant), data= IPET.df)
step(med.fit_1v2_zcpHP) #remove dummy_img 
#Final model
med.fit_1v2_FinalHP<- lmer(Perspective.response~dummy_ident +dummy_img+(dummy_ident|participant), data= IPET.df)
summary(med.fit_1v2_FinalHP)
```
###Path b and c'
```{r}
#Identify vs Imagine: path b
out.fit_0v2_zcpHP<- lmer(help.response~dummy_est +dummy_img+Perspective_C+(dummy_est +dummy_img +Perspective_C||participant), data= IPET.df)
step(out.fit_0v2_zcpHP) #retain all
#Final model
...
summary(out.fit_0v2_finalHP)

#Estimate vs Imagine: path b
out.fit_1v2_zcpHP<- lmer(help.response~dummy_ident +dummy_img+Perspective_C+(dummy_ident +dummy_img +Perspective_C||participant), data= IPET.df)
step(out.fit_1v2_zcpHP) #retain all
#Final model
out.fit_1v2_finalHP<- lmer(help.response~dummy_ident +dummy_img+Perspective_C+(dummy_ident +dummy_img |participant), data= IPET.df)#remove Perspective_C convergence failure
summary(out.fit_1v2_finalHP)
```
...
detach (package:lmerTest, unload=T)
#Identify vs Imagine
med.fit_0v2_zcpTH<- lmer(help.response~dummy_est +dummy_img+(dummy_est +dummy_img||participant), data= IPET.df)
step(med.fit_0v2_zcpTH) #retain all 
#final model
med.fit_0v2_finalTH<- lmer(help.response~dummy_est +dummy_img+(dummy_est +dummy_img|participant), data= IPET.df)
med.fit_0v2_finalTH<- update(med.fit_0v2_finalTH, .~., start= getME(med.fit_0v2_finalTH, 'theta')) #one refit
summary(med.fit_0v2_finalTH)

#Estimate vs Imagine
med.fit_1v2_zcpTH<- lmer(help.response~dummy_ident +dummy_img+(dummy_ident +dummy_img||participant), data= IPET.df)
step(med.fit_1v2_zcpTH) #retail all
#final model
med.fit_1v2_finalTH<- lmer(help.response~dummy_ident +dummy_img+(dummy_img+dummy_ident|participant), data= IPET.df)
summary(med.fit_1v2_finalTH)
```
###Path b and c'
```{r}
#Identify vs Imagine
out.fit_0v2_zcpTH<- lmer(trust.response~dummy_est +dummy_img +Help_C+(dummy_est +dummy_img +Help_C||participant), data= IPET.df)
step(out.fit_0v2_zcpTH) #remove dummy_img and dummy_est
#final model
out.fit_0v2_finalTH<- lmer(trust.response~dummy_est +dummy_img +Help_C+(Help_C|participant), data= IPET.df)
summary(out.fit_0v2_finalTH)

#Estimate vs Imagine
out.fit_1v2_zcpTH<- lmer(trust.response~dummy_ident +dummy_img +Help_C+(dummy_ident +dummy_img +Help_C||participant), data= IPET.df)
step(out.fit_1v2_zcpTH) #remove dummy_ident and dummy_img
#final model
out.fit_1v2_finalTH<- lmer(trust.response~dummy_ident +dummy_img +Help_C+(Help_C|participant), data= IPET.df)
summary(out.fit_1v2_finalTH)
```
